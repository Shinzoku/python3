...
for i in range(len(cards)):
    print(f'Carte: {i + 1} couleur: {cards[i][1]}, valeur: {cards[i][2]}')
...
cards.remove(card)
alice_cards.append(card)
print(f'le nombre de cartes restant dans le paquet: {len(cards)}')
cards.extend(alice_cards)
alice_cards.clear()
...
print(f'Les cartes que possède bob: {bob_cards}')
print(f'le nombre de cartes restant dans le paquet: {len(cards)}')
...
for i in range(3):
    ...
    for k in range(len(alice_cards)):
        symbol_occur = alice_cards.count('coeur')
        if alice_cards[k][2] == 3 or symbol_occur == 2:
            alice_win += 1
    ...
    for k in range(len(bob_cards)):
        symbol_occur = bob_cards.count('pique')
        if bob_cards[k][2] == 7 or symbol_occur == 2:
            bob_win += 1
